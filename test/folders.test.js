...
describe.only('Noteful API - Folders', function() {
  ...
  describe('GET /api/folders/:id', function() {
    it('should return correct folder', function() {
      ...
      const apiPromise = chai
        .request(app)
        .get('/api/folders/222222222222222222222201')
        .set('Authorization', `Bearer ${token}`);

      return Promise.all([dbPromise, apiPromise]).then(([_data, res]) => {
        expect(res).to.have.status(200);
        expect(res).to.be.json;
        ...
        expect(res.body.id).to.equal(_data.id);
        expect(res.body.name).to.equal(_data.name);
        expect(new Date(res.body.createdAt)).to.eql(_data.createdAt);
        expect(new Date(res.body.updatedAt)).to.eql(_data.updatedAt);
      });
    });
    ...
    it('should respond with a 400 for an invalid id', function() {
      ...
      const apiPromise = chai
        .request(app)
        .get('/api/folders/NOT-A-VALID-ID1')
        .set('Authorization', `Bearer ${token}`);
