...
describe('Noteful API - Notes', function() {
  ...
  describe('GET /api/notes', function() {
    ...
    it('should return correct search results for a title search', function() {
      ...
      const dbPromise = Note.find({
        $and: [{ userId: user.id }, { $or: [{ title: re }, { content: re }] }]
      }).sort({ updatedAt: 'desc' });
  ...
  describe('PUT /api/notes/:id', function() {
    ...
    it('should update the note when provided a valid folderId', function() {
      ...
      return Promise.all([Folder.findOne({ userId: user.id }), Note.findOne({ userId: user.id })])
        .then(([folder, note]) => {
    ...
    it('should update the note when provided a valid tag', function() {
      ...
      return Promise.all([Tag.findOne({userId: user.id}), Note.findOne({userId: user.id})])
        .then(([tag, note]) => {
        ...
        .then(function(res) {
          ...
          expect(res.body.title).to.equal(data.title);
          expect(res.body.content).to.equal(data.content);
          expect([res.body.tags[0].id]).to.deep.equal(updateItem.tags);
          expect(new Date(res.body.createdAt)).to.eql(data.createdAt);
          expect(new Date(res.body.updatedAt)).to.greaterThan(data.updatedAt);
        });
    });
